...
export class TabsComponent implements OnInit {
  ...
  @ViewChild("tabSlot")
  tabSlot: ElementRef<HTMLSlotElement>;
  @ViewChild("panelSlot")
  panelSlot: ElementRef<HTMLSlotElement>;
  ...
  linkPanels() {
    ...
    tabs.forEach(tab => {
      ...
      if (panel.tagName.toLowerCase() !== "demo-panel") {
        console.error(`Tab #${tab.id} is not a` + `sibling of a <demo-panel>`);
        return;
      }
  ...
  selectTab(newTab: TabElement) {
    ...
    if (!newPanel) throw new Error(`Panel does not exist!`);
  ...
  allPanels() {
    return this.panelSlot.nativeElement.assignedElements() as Array<
      PanelElement
    >;
  }
}
